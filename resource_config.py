...
def _find_config_path(config_path=None):
    ...
    if getattr(sys, "frozen", False):
        external_path = Path(sys.executable).resolve().parent / DEFAULT_CONFIG_RELATIVE_PATH
        if external_path.is_file():
            return external_path

    return (_application_base_path() / DEFAULT_CONFIG_RELATIVE_PATH).resolve()
...
class ResourceConfig:
    ...
    def load(cls, config_path=None):
        source_path = _find_config_path(config_path)
    ...
    def _load_sprites(self, sprites):
        ...
        for name in REQUIRED_SPRITES:
            ...
            result[name] = self._load_sprite_spec(spec, label)
        return result
